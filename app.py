...
try:
    if "GEMINI_API_KEY" in st.secrets:
        ...
        # Modelo actualizado para máxima compatibilidad y evitar el error 404
...
# --- CONSTANTES ---
# Nombres de técnicos omitidos por configuración de privacidad
LISTA_TECNICOS = ["Técnico 1", "Técnico 2", "Técnico 3"] 
...
if st.button("🤖 Autocompletar con IA"):
    if ia_disponible and f_rev_fisica:
        with st.spinner("Gemini está procesando..."):
            try:
                prompt = f"Como técnico experto, analiza esta revisión física: '{f_rev_fisica}' del producto '{f_prod}'. Genera: 1- Pasos técnicos de revisión electro-mecánica. 2- Observaciones. Formato estricto: ELECTRO: [texto] OBS: [texto]"
...
st.text_area("3. Revisión electro-electrónica-mecanica", value=st.session_state.ai_electro, key="electro_input")
st.text_area("4. Observaciones", value=st.session_state.ai_obs, key="obs_input")
f_concl = st.text_area("5. Conclusiones", value=TEXTOS_CONCLUSIONES.get(tipo_rep, ""), height=150)

# --- IMÁGENES Y GENERACIÓN ---
...
if st.button("💾 GENERAR ARCHIVOS", use_container_width=True):
    ...
    # Recuperamos los valores actualizados de los text_area
    rev_electro_final = st.session_state.electro_input if 'electro_input' in st.session_state else st.session_state.ai_electro
    obs_final = st.session_state.obs_input if 'obs_input' in st.session_state else st.session_state.ai_obs

    datos = {"orden": orden_id, "cliente": f_cliente, "factura": f_fac, "fecha_factura": f_fec_fac, "producto": f_prod, "serie": f_serie, "tecnico": f_tecnico, "realizador": f_realizador, "fecha_hoy": date.today(), "rev_fisica": f_rev_fisica, "ingreso_tec": f_ingreso_tec, "rev_electro": rev_electro_final, "observaciones": obs_final, "conclusiones": f_concl, "tipo_reporte": tipo_rep}
