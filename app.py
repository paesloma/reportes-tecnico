...
import os
from groq import Groq  # <-- Agregado para IA
...
def generar_txt_contenido(datos):
    fac_txt = "STOCK" if str(datos['factura']).strip() == "0" else datos['factura']
    return (
        f"Estimados\n\nMe dirijo a usted para indicar el status de estado de la garantía del siguiente producto:\n\n"
        f"CLIENTE: {datos['cliente']}\nFACTURA: {fac_txt}\nFECHA DE FACTURA: {datos['fecha_factura']}\n"
        f"ORDEN DE SERVICIO: {datos['orden']}\nSERIE/CÓDIGO: {datos['serie']}\nPRODUCTO: {datos['producto']}\n"
...
# --- BLOQUE IA (GROQ) PARA OBSERVACIONES ---
st.markdown("---")
st.markdown("### 🤖 Asistente de Observaciones (Groq)")
clave_groq = st.text_input("Clave API Groq", type="password")
falla_breve = st.text_input("Describe la falla brevemente para la IA:")

if st.button("✨ Generar Observación con IA"):
    if clave_groq and falla_breve:
        try:
            client = Groq(api_key=clave_groq)
            chat_completion = client.chat.completions.create(
                messages=[
                    {"role": "system", "content": "Eres un perito técnico. Redacta una observación técnica profesional y concisa basada en la falla descrita."},
                    {"role": "user", "content": f"Producto: {f_prod}. Falla: {falla_breve}"}
                ],
                model="llama-3.3-70b-versatile",
            )
            st.session_state.texto_ia_obs = chat_completion.choices[0].message.content
            st.rerun()
...
# --- SECCIÓN DE IMÁGENES CON BORRADO ---
st.markdown("---")
...
if st.session_state.imagenes_guardadas:
    for idx, img_data in enumerate(st.session_state.imagenes_guardadas):
        ...
        with col_desc:
            st.session_state.imagenes_guardadas[idx]["desc"] = st.text_input(f"Descripción #{idx+1}", value=img_data["desc"], key=f"input_{idx}")
        with col_del:
            st.write(" ")
...
if st.button("💾 GENERAR ARCHIVOS", use_container_width=True):
    ...
    for img_item in st.session_state.imagenes_guardadas:
        try:
            p_img = PilImage.open(BytesIO(img_item["bytes"]))
...
if st.session_state.pdf_data:
    ...
    c1, c2 = st.columns(2)
    with c1: st.download_button("Descargar PDF", data=st.session_state.pdf_data, file_name=f"Informe_{orden_id}.pdf", mime="application/pdf", use_container_width=True)
    with c2: st.download_button("Descargar TXT", data=st.session_state.txt_data, file_name=f"Status_{orden_id}.txt", mime="text/plain", use_container_width=True)
